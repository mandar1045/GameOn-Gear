...
class EnhancedUserDatabase {
  ...
  private readonly MAX_USERS = 5000; // Increased capacity to prevent limit errors
  private readonly BACKUP_INTERVAL = 5 * 60 * 1000; // 5 minutes
  private backupTimer: NodeJS.Timeout | null = null;
  
  // Define the single admin email
  private readonly ADMIN_EMAIL = 'mandarjoshi1045@gmail.com';
  ...
  private initializeDefaultUsers(): void {
    // Only create the single admin user
    const adminUser = {
    ...
    if (!this.emailIndex.has(adminUser.email)) {
      try {
        this.createUser(adminUser);
      } catch (error) {
        console.warn(`Failed to create admin user ${adminUser.email}:`, error);
      }
    }
    ...
    if (!this.emailIndex.has(demoUser.email)) {
      try {
        ...
        console.warn(`Failed to create demo user ${demoUser.email}:`, error);
      }
    }

    // Only generate sample users if we have room and don't already have many users
    const currentUserCount = this.users.size;
    const targetSampleUsers = Math.min(20, this.MAX_USERS - currentUserCount - 100); // Leave buffer space
    
    if (currentUserCount < 25 && targetSampleUsers > 0) {
      this.generateSampleUsers(targetSampleUsers);
    }
  }
  ...
  private generateSampleUsers(count: number): void {
    ...
    while (created < count && attempts < maxAttempts) {
      ...
      const email = `${firstName.toLowerCase()}.${lastName.toLowerCase()}${attempts}@${domains[Math.floor(Math.random() * domains.length)]}`;
      ...
      if (!this.emailIndex.has(email)) {
        try {
          ...
          console.warn(`Failed to create sample user ${email}:`, error);
          // If we hit the limit, stop trying
          if (error instanceof Error && error.message.includes('Maximum user limit')) {
            break;
          }
        }
      }
  ...
  createUser(userData: {
    ...
    // Determine role based on email - only the specific admin email gets admin role
    let userRole: User['role'] = 'user';
    if (userData.email.toLowerCase() === this.ADMIN_EMAIL.toLowerCase()) {
      userRole = 'admin';
    } else if (userData.role && userData.role !== 'admin') {
      // Allow moderator role for non-admin emails, but never admin
  ...
  updateUserRole(userId: string, newRole: User['role']): boolean {
    ...
    // Prevent changing admin role for non-admin emails
    if (newRole === 'admin' && user.email.toLowerCase() !== this.ADMIN_EMAIL.toLowerCase()) {
      return false;
    }

    // Prevent removing admin role from the admin email
    if (user.email.toLowerCase() === this.ADMIN_EMAIL.toLowerCase() && newRole !== 'admin') {
      return false;
    }
  ...
  deactivateUser(userId: string): boolean {
    ...
    if (user.email.toLowerCase() === this.ADMIN_EMAIL.toLowerCase()) {
      return false;
    }
  ...
  importUsers(csvData: string): { imported: number; errors: string[] } {
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        if (this.users.size >= this.MAX_USERS) {
          errors.push(`Line ${i + 1}: Maximum user limit reached`);
          break;
        }
  ...
  updateUser(id: string, updates: Partial<User>): User | null {
    ...
    // Handle role change with admin protection
    if (updates.role && updates.role !== user.role) {
      // Prevent changing admin role for non-admin emails
      if (updates.role === 'admin' && user.email.toLowerCase() !== this.ADMIN_EMAIL.toLowerCase()) {
        delete updates.role; // Remove the role update
      }
      
      // Prevent removing admin role from the admin email
      if (user.email.toLowerCase() === this.ADMIN_EMAIL.toLowerCase() && updates.role !== 'admin') {
        delete updates.role; // Remove the role update
      }
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const isAdmin = (): boolean => {
    return user ? userDB.isUserAdmin(user.email) : false;
  };
